...
main() {
  ...
  comm -13 <(yq eval '.linters.intentionally-disabled[].name' $config_file | sort) <(golangci-lint linters | awk -F ':' '{ if (disabled==1) print $1 }; /Disabled by your/ { disabled=1 }' | sort)
  echo
  ...
  comm -23 <(yq eval '.linters.intentionally-disabled[].name' $config_file | sort) <(golangci-lint linters | awk -F ':' '{ if (disabled==1) print $1 }; /Disabled by your/ { disabled=1 }' | sort)
