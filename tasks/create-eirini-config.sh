...
create_config() {
  ...
  goml set -f configs/opi.yaml -p opi.cc_uploader_ip -v "$DIRECTOR_IP"
  ...
  goml set -f configs/opi.yaml -p opi.external_eirini_address -v "registry-$DIRECTOR_NAME.$KUBE_ENDPOINT:80"
  goml set -f configs/opi.yaml -p opi.eirini_address -v "opi-$DIRECTOR_NAME.$KUBE_ENDPOINT:80"
}
